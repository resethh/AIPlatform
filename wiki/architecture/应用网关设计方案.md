...
需求场景：
AI中台链接外部应用服务，比如crm、企业微信，通过调用api方式实现功能。
存在以下问题：
1. 

## 一句话定位
综合 参考Anthropic Claude 团队的mcp管理和上下文优化方法，以及hiclaw的角色架构划分。

hiclaw：通过  **Agent Manager- Agent Worker 双角色 + AI Gateway 凭证代理**，实现"工具全接入 + 真实凭证零暴露"——Worker 只拿可吊销的 consumer token，真实 API key / Bearer token / Service Token 永不进入 Worker 进程。

---

## 对接生产系统，为什么选 MCP，而不是 Direct API 或 CLI
...
用户
  ↓
[^1]Manager agent（管控平面）
  │ 1. 用户描述 API , 并上传应用系统凭证 ，进行解析，凭证存在数据库（用户上传会带AI中台的凭证信息，存在两个凭证对应关系）
  │ 2. 根据解析内容进行分析，生成 MCP Server YAML 配置和相应skill；MCP Server YAML和skill存到对象存储
  │ 3. skill自动走审批流程（调取别的接口不用开发）
  │ 4. MCP Server YAML发送给gateway
  ↓
 Gateway（应用网关层）
  │ 功能1：接收MCP Server YAML 配置，部署到 Gateway
  │ 功能2：接收worken agent的请求，代理所有 MCP 调用，按 token 做权限校验，从数据库获取凭证或动态获取token并缓存，记录审计日志
  │ 
  │ 
  ↓
Worker agent（执行平面）
...
https://higress.io/docs/ai/mcp-server/#_top

![[Pasted image 20260428144612.png]]
...
sequenceDiagram
    ...
    participant McPorter as mcporter CLI

    Note over Admin,McPorter: 阶段一：接入新 API
    Admin->>Manager: 提供 Swagger/curl
    Manager->>Gateway: 生成并部署 MCP Server
    Gateway-->>Manager: MCP Server 就绪
    Manager->>Worker: 通知有新工具可用

    Note over Admin,McPorter: 阶段二：Worker 初始化
    Worker->>McPorter: mcporter list --schema
    McPorter->>Gateway: 发现工具
    Gateway-->>McPorter: 返回工具 Schema
    McPorter-->>Worker: 返回工具信息
    Worker->>Worker: 自动生成 SKILL.md

    Note over Admin,McPorter: 阶段三：运行时调用
    Worker->>McPorter: mcporter call server.tool args
    McPorter->>Gateway: MCP 协议请求 + Consumer Token
    Gateway->>Gateway: 注入真实凭证
    Gateway->>McPorter: 返回结果
    McPorter-->>Worker: 格式化输出
```
...
sequenceDiagram
    ...
    Note over Admin,McPorter: 阶段一：接入新 API
    Admin->>Manager: 提供 Swagger/curl
    Manager->>Manager: 生成 MCP Server 配置
    Manager->>Manager: 自动生成 SKILL.md
    Manager->>Gateway: 部署 MCP Server
    Gateway-->>Manager: MCP Server 就绪
    Manager->>Worker: 分发 SKILL.md

    Note over Admin,McPorter: 阶段二：Worker 初始化
    Worker->>Worker: 读取 SKILL.md
    Worker->>McPorter: mcporter list --verify
    McPorter->>Gateway: 验证工具可达性
    Gateway-->>McPorter: 返回健康状态
    McPorter-->>Worker: 确认可用
...
### 待确认
实现方式：动态凭证获取在mcp或者skill。应用端静态凭证获取在mcp或skill。应该都在mcp.

1. 端点-mcp tool一对一，在skill里写上认证方式和缓存机制，比如skill.md里包括每次执行前去redis进行查询，没有调用查询token接口并记录到redis里。
2. 认证方式在

skill: 此时要带上中台api key
查询用户权限：
curl: post/crm ，调用应用网关接口
