...
通知 Skill Owner / Admin 审核
    ↓
批准：写入 skills 表 + OSS，status=canary
拒绝：归档，附理由
```
...
多租户场景下 `/skill-name` 需按当前用户的 `owner_node` 祖先路径过滤可见 Skill，同名 Skill 走**就近覆盖**原则（部门级覆盖集团级）。
...
| 层 | 替代方案 | key |
|---|---|---|
| Level 0 索引 | Redis，TTL 5 分钟，skill_manage 写操作主动失效 | `skills:index:{tenant_id}:{owner_node}` |
...
```
skill_manage(patch/edit) → skill_proposals 表（action=patch, base_version=x.y.z）
    ↓
审核
    ↓
批准：生成新版本号（patch 递增），写入 OSS 新路径，skills.version 更新
```
...
- **租户可见性检查**：Skill 的 `owner_node` 不在当前用户祖先路径上 → FORBIDDEN
...
| 定期审计 | 每日批量扫描所有 stable 版本，发现新型注入模式追溯预警 |

---

## 13. 多租户组织继承（中台新增）

Hermes 无此能力。中台采用 PostgreSQL `ltree` 物化路径索引支持树形继承：
...
**查询示例**（当前用户 `owner_node=acme.sales.east`）：

```sql
SELECT DISTINCT ON (name) *
FROM skills
WHERE tenant_id = $1
  AND owner_node @> 'acme.sales.east'::ltree   -- ltree 祖先匹配
  AND status = 'stable'
ORDER BY name, nlevel(owner_node) DESC;         -- 就近覆盖（路径层数越深越优先）
```
...
Hermes 直接覆盖文件，无版本管理。中台采用语义化版本 + 灰度发布：
...
**灰度路由键**：`tenant_id`，同一租户在灰度期间始终用同一版本（避免用户看到同一 Skill 行为抖动）。

**OSS 历史保留**：所有版本的对象都永久保留，`skill_versions` 表记录每次发布的 oss_key，支持一键回滚。
